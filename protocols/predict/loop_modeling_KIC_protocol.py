...
if len(sys.argv)!=8:
    print 'Usage: python loonkic.py <project> <Template> <target_sequence> <template_sequence> <start> <end> <nstruct>' 
    sys.exit(0)
#global variables 
pwd = os.getcwd()
...
f.write('-s ../starting_files/start_input/%s_MinPack.pdb\n' % PDB)
...
f.write('score_jd2.linuxgccrelease -in:file:s *.pdb -out:pdb -database $ROSETTA_DATABASE >score.log\n')
f.write("awk '{print$2,$21}' score.sc |sort -nk1 |head -n1 |sed 's/_0001/.pdb/' |awk '{print$2}' >native\n")
f.write('for f in *_0001.pdb;do mv -f  $f "`echo $f |sed s/_0001//`";done\n')
f.write('score_vs_rmsd_full.py -o %s -m -n `cat native` *pdb\n' % pdir)
f.write("awk '{print$2,$3,$4,$19,$1}' %s_align* |sort -nk4|sed 's/ /,/g'|sed '$d' |sed '1 i\ca_rmsd,bb_rmsd,all_atom,score,model'> %s_sorted.csv\n" % (pdir,pdir))
f.write("awk '{print$2,$3,$4,$19,$1}' *_align* |sort -nk4|awk '{print$5}' >byscore\n")
f.write('move.bash\n')
f.close()
