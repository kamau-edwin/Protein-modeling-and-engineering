...
f.write('%s 1 %s\n%s_relax.pdb 1 %s' % (pdir, full_target_sequence, PDB, full_template_sequence))
f.close()
...
f.write('-in:file:psipred_ss2 ' + pdir + '.psipred_ss2\n')
...
f.write('mpirun -np $NSLOTS minirosetta.mpi.linuxgccrelease @' + pdir + '.flags -database $ROSETTA_DATABASE &>log.out\n')
...
f.write('#$ -N analyze\n')
f.write('#$ -cwd\n')
f.write('#$ -pe openmpi 50-64\n')
f.write('#$ -j y\n')
f.write('#$ -o '+ pdir + '/'+ pdir +'.out/Combined/processing.out\n')
f.write('cd '+ pdir + '/' + pdir +'.out/Combined\n')
f.write('module load openmpi/gcc/64/1.4.5\n')
f.write('mpirun -np $NSLOTS combine_silent.mpi.linuxgccrelease -in:file:silent ../*.silent.* -out:file:silent Combined.sile -out:file:silent_struct_type binary -silent_read_through_errors -database $ROSETTA_DATABASE >&Combine.log\n')
f.write('Clusteranscore.py Combined.sile per.silent -1 0.1\n')
f.write('rm -f ../*.silent.*\n')
f.write('mkdir -p clust\n')
f.write('cd clust\n')
f.write('mpirun -np $NSLOTS  cluster.mpi.linuxgccrelease -in:file:silent ../*.silent -in:file:fullatom -in:file:native $INPUT/'+ PDB + '_relax.pdb -cluster:gdtmm -cluster:radius -1 -cluster:population_weight 0.0 -cluster:sort_groups_by_energy -no_optH -out:pdb -database $ROSETTA_DATABASE >& cluster.log\n')
f.write('score.linuxgccrelease -in:file:s *.pdb -in:file:native $INPUT/'+ PDB + '_relax.pdb -out:file:scorefile score.sc -database $ROSETTA_DATABASE >score.log\n')
# move top20 structures based on score
